"""
CircleID — Pen Classification v15b
v9 + SAM (Sharpness-Aware Minimization) optimizer

Key change from v9:
1. SAM wraps AdamW — seeks flatter minima for better generalization
   - Two forward passes per step (2x slower)
   - But finds solutions that generalize better to unseen data
...
class CFG:
    ...
    output_dir: Path = BASE_DIR / "outputs_v15b"
    checkpoint_dir: Path = BASE_DIR / "checkpoints_v15b"
    num_pens: int = 8
    # -- Backbone --
...
def train_one_fold(fold, train_df, val_df, cfg, pen2idx):
    ...
    optimizer = SAM(
        [
            {"params": backbone_params, "lr": cfg.lr_backbone, "rho": 0.05},
            {"params": head_params, "lr": cfg.lr_head, "rho": 0.05},
        ],
    ...
    warmup = LinearLR(optimizer.base_optimizer, start_factor=0.01, total_iters=warmup_steps)
    cosine = CosineAnnealingLR(optimizer.base_optimizer, T_max=max(1, total_steps - warmup_steps))
    scheduler = SequentialLR(optimizer.base_optimizer, [warmup, cosine], milestones=[warmup_steps])
    ...
    for epoch in range(cfg.epochs):
        ...
        for step, (images, labels) in enumerate(pbar):
            ...
            # SAM first forward pass — compute loss at current point
            ...
            # SAM second forward pass — compute loss at perturbed point
            with torch.amp.autocast("cuda"):
                logits_2 = model(images)
                loss_2 = criterion(logits_2, labels)
            loss_2.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), cfg.grad_clip)
            optimizer.second_step()  # step using perturbed gradients
            optimizer.zero_grad()
            scheduler.step()
...
def run_inference(cfg, pen2idx):
    ...
    sub_path = cfg.output_dir / "submission_v15b.csv"
    ...
    np.save(cfg.output_dir / "test_logits_v15b.npy", all_logits)
    print(f"  Logits saved: {cfg.output_dir / 'test_logits_v15b.npy'}")
...
def main():
    ...
    print(f"CircleID Pen Classification v15b — ConvNeXt-V2 Tiny + SAM Optimizer")
