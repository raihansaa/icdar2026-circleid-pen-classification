"""
CircleID — Pen Classification v6
DINOv2 + Expanded LoRA + Rich Ink Features + ArcFace + Same-Pen Mixup + K-Fold

Key changes from v5:
1. Removed rotation augmentation — two resampling passes (warpAffine + resize)
   destroy micro-texture ink bleed signal. Circles are rotationally symmetric
   so rotation teaches nothing useful.
2. Raised crop_scale_min 0.5 → 0.85 — less resize distortion on ~100px circles
3. Added H-flip TTA at inference — runs normal + flipped pass and averages logits
...
class PenConfig:
    ...
    output_dir: Path = BASE_DIR / "outputs_v6"
    checkpoint_dir: Path = BASE_DIR / "checkpoints_v6"
    ...
    pseudo_csv: str = "outputs_v6/pseudo_labels.csv"
    pseudo_soft_labels: str = "outputs_v6/pseudo_soft_labels.npy"
...
class PenDataset(Dataset):
    def __init__(self, df, indices, data_dir, pen2idx, writer2idx,
                 ...
                 pseudo_start_idx=None, soft_labels=None, num_pens=8):
        self.df = df
        ...
        self.pseudo_start_idx = pseudo_start_idx
    ...
    def __getitem__(self, idx):
        ...
        if self.pseudo_start_idx is not None and global_idx >= self.pseudo_start_idx:
            pseudo_idx = global_idx - self.pseudo_start_idx
            soft_target = torch.from_numpy(self.soft_labels[pseudo_idx]).float()
            is_pseudo = True
        else:
            soft_target = torch.zeros(self.num_pens, dtype=torch.float32)
            is_pseudo = False

        return {"image": img_tensor, "mask": mask_tensor, "ink_feats": ink_feats,
                "pen_label": pen_label, "writer_id": writer_id,
                "soft_target": soft_target, "is_pseudo": is_pseudo}
...
def train_one_epoch(model, loader, optimizer, scheduler, device, epoch, cfg,
    ...
    for step, batch in enumerate(loader):
        ...
        # Only mix real samples — pseudo samples have uncertain writer identity
        real_mask = ~is_pseudo
        if epoch >= cfg.warmup_epochs and np.random.random() < cfg.mixup_prob:
            if real_mask.all():
                ...
                # Mix only within real samples, keep pseudo untouched
                r_imgs, r_masks, r_feats = apply_same_pen_mixup(
                    imgs[real_mask], masks[real_mask], ink_feats[real_mask],
                    labels[real_mask], wids[real_mask], cfg.mixup_alpha)
                imgs[real_mask] = r_imgs
                masks[real_mask] = r_masks
                ink_feats[real_mask] = r_feats
            n_mixup += 1
        ...
        with torch.amp.autocast("cuda", enabled=scaler is not None):
            ...
            # Classification loss: split real (hard CE) vs pseudo (soft KL)
            if cfg.use_pseudo and is_pseudo.any():
                n_real = real_mask.sum().float()
                n_pseudo = is_pseudo.sum().float()
                if real_mask.any():
                    loss_real = ce_criterion(logits[real_mask], labels[real_mask])
                else:
                    loss_real = torch.tensor(0.0, device=device)
                pseudo_logprobs = F.log_softmax(logits[is_pseudo], dim=1)
                loss_pseudo = F.kl_div(
                    pseudo_logprobs, soft_targets[is_pseudo], reduction='batchmean')
                total_w = n_real + cfg.pseudo_weight * n_pseudo
                loss_arc = (loss_real * n_real +
                            cfg.pseudo_weight * loss_pseudo * n_pseudo) / (total_w + 1e-8)
            else:
                loss_arc = ce_criterion(logits, labels)

            # Contrastive loss: real samples only
            if lam_con > 0:
                if cfg.use_pseudo and is_pseudo.any():
                    if real_mask.sum() >= 2:
                        loss_con = pen_contrastive_loss(
                            pen_emb[real_mask], labels[real_mask],
                            wids[real_mask], cfg.contrast_tau)
                    else:
                        loss_con = torch.tensor(0.0, device=device)
                else:
                    loss_con = pen_contrastive_loss(
                        pen_emb, labels, wids, cfg.contrast_tau)
            else:
                loss_con = torch.tensor(0.0, device=device)

            # Adversarial loss: real samples only 
            if cfg.use_pseudo and is_pseudo.any():
                if real_mask.any():
                    loss_adv = F.cross_entropy(
                        writer_logits[real_mask], wids[real_mask])
                else:
...
def train_fold(fold, train_idx, val_idx, df, data_dir, pen2idx, writer2idx,
               cfg, device, pseudo_start_idx=None, soft_labels=None):
    print(f"\n{'=' * 70}")
    ...
    n_pseudo_in_fold = sum(1 for i in train_idx if pseudo_start_idx and i >= pseudo_start_idx) if pseudo_start_idx else 0
    print(f"  Train: {len(train_idx)} ({len(train_writers)} writers, "
          f"{n_pseudo_in_fold} pseudo), "
    ...
    train_ds = PenDataset(
        ...
        pseudo_start_idx=pseudo_start_idx, soft_labels=soft_labels,
        num_pens=cfg.num_pens)
    val_ds = PenDataset(
...
def run_inference_fold(fold, cfg, device, test_ds, num_writers, use_amp):
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            masks_flip = torch.flip(masks, dims=[-1])
            with torch.amp.autocast("cuda", enabled=use_amp):
                logits_flip = model(imgs_flip, masks_flip, ink_feats, cfg.eps)
            logits_flip = logits_flip.view(B, K, -1).mean(dim=1)
            all_logits_flip.append(logits_flip.cpu().numpy())
    ...
    del model
    torch.cuda.empty_cache()
    # Average normal + H-flip logits
    normal = np.concatenate(all_logits, axis=0)
    flipped = np.concatenate(all_logits_flip, axis=0)
    return (normal + flipped) / 2.0
...
if __name__ == "__main__":
    ...
    print(f"\n{'=' * 70}")
    print(f"CircleID Pen v6 — NoRotation+CropMin85+HflipTTA+{cfg.n_folds}-Fold")
    print(f"{'=' * 70}")
    ...
    print(f"  Rotation: {cfg.random_rotation_deg}° (v6: disabled)")
    print(f"  Crop scale: [{cfg.crop_scale_min}, {cfg.crop_scale_max}] (v6: min raised)")
    print(f"  H-flip TTA: enabled at inference")
    print(f"  Pseudo-labels: {'ENABLED (weight={})'.format(cfg.pseudo_weight) if cfg.use_pseudo else 'disabled'}")
    ...
    # ── Pseudo-labeling: merge pseudo data ────────────────────────
    pseudo_start_idx = None
    soft_labels = None
    if cfg.use_pseudo:
        pseudo_path = Path(cfg.pseudo_csv)
        soft_path = Path(cfg.pseudo_soft_labels)
        if not pseudo_path.exists() or not soft_path.exists():
            print(f"ERROR: Pseudo-label files not found. Run without --pseudo first")
            print(f"  (generates pseudo-labels automatically), then re-run with --pseudo.")
            sys.exit(1)
        df_pseudo = pd.read_csv(pseudo_path)
        soft_labels = np.load(soft_path)
        pseudo_start_idx = len(df)
        df = pd.concat([df, df_pseudo], ignore_index=True)
        # Add pseudo writer_id (-1) to mapping
        writer2idx[-1] = len(writer2idx)
        print(f"  Pseudo-labels: {len(df_pseudo)} samples (threshold conf)")
        print(f"  Total training pool: {len(df)} (real + pseudo)")
    ...
    # ── Fold splitting ────────────────────────────────────────────
    if cfg.use_pseudo and pseudo_start_idx is not None:
        # Real samples: standard GroupKFold by writer
        real_indices = np.arange(pseudo_start_idx)
        pseudo_indices = np.arange(pseudo_start_idx, len(df))
        gkf = GroupKFold(n_splits=cfg.n_folds)
        real_folds = list(gkf.split(
            real_indices, df.iloc[real_indices]["pen_id"].values,
            groups=df.iloc[real_indices]["writer_id"].values))
        # Pseudo samples always in training, never validation
        folds = []
        for train_real, val_real in real_folds:
            train_idx = np.concatenate([real_indices[train_real], pseudo_indices])
            val_idx = real_indices[val_real]
            folds.append((train_idx, val_idx))
        print(f"  Folds: pseudo samples added to all training splits")
    else:
        gkf = GroupKFold(n_splits=cfg.n_folds)
    ...
    if not args.infer_only:
        ...
        for fold_idx in folds_to_train:
            ...
            acc, f1 = train_fold(fold_idx, train_idx, val_idx,
                                  ...
                                  pseudo_start_idx=pseudo_start_idx,
    ...
    out_path = Path(cfg.output_dir) / "submission_pen_v6.csv"
    ...
    logits_path = Path(cfg.output_dir) / "test_logits_v6.npy"
    np.save(logits_path, final_logits)

    probs = sp_softmax(final_logits, axis=1)
    ...
    print(f"{'=' * 70}")
    print(f"  Confidence >= {THRESHOLD}: {conf_mask.sum()} / {len(df_test)} "
          f"({100*conf_mask.mean():.1f}%)")

    pseudo_df = df_test[conf_mask].copy()
    pseudo_df["pen_id"] = [idx2pen[int(i)] for i in pred_idx[conf_mask]]
    pseudo_df["writer_id"] = -1
    pseudo_df["confidence"] = max_conf[conf_mask]

    for pen in sorted(pseudo_df["pen_id"].unique()):
        n = (pseudo_df["pen_id"] == pen).sum()
        avg_c = pseudo_df[pseudo_df["pen_id"] == pen]["confidence"].mean()
        print(f"    Pen {pen}: {n:5d} samples, avg conf {avg_c:.4f}")

    pseudo_df.to_csv(Path(cfg.output_dir) / "pseudo_labels.csv", index=False)
    np.save(Path(cfg.output_dir) / "pseudo_soft_labels.npy", probs[conf_mask])
    print(f"  Saved pseudo-labels → {cfg.output_dir}/pseudo_labels.csv")
    print(f"  Re-run with --pseudo to train with pseudo-labels")
    print("\nDone.")
