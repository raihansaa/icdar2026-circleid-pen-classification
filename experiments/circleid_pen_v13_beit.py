"""
CircleID — Pen Classification v13
BEiT-Base (Masked Image Modeling pretrained, IN22K→IN1K, 86M params)

5th model for ensemble diversity. BEiT uses masked image modeling (like BERT for images).
Different pretraining from all existing models.
...
class CFG:
    ...
    output_dir: Path = BASE_DIR / "outputs_v13"
    checkpoint_dir: Path = BASE_DIR / "checkpoints_v13"
    num_pens: int = 8
    # -- Backbone --
    backbone: str = "beit_base_patch16_224.in22k_ft_in22k_in1k"
    image_size: int = 336  
...
def run_inference(cfg, pen2idx):
    ...
    sub_path = cfg.output_dir / "submission_v13.csv"
    ...
    np.save(cfg.output_dir / "test_logits_v13.npy", all_logits)
    print(f"  Logits saved: {cfg.output_dir / 'test_logits_v13.npy'}")
...
def main():
    ...
    print(f"CircleID Pen Classification v13 — BEiT-Base")
