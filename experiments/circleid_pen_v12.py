"""
CircleID — Pen Classification v12
ConvNeXt-V2 Tiny + Focal Loss + MixUp/CutMix

Same as v9 but with MixUp and CutMix augmentation.
MixUp blends images and labels, helping learn better decision boundaries for pen 3/7.
...
class CFG:
    ...
    output_dir: Path = BASE_DIR / "outputs_v12"
    checkpoint_dir: Path = BASE_DIR / "checkpoints_v12"
    num_pens: int = 8
    # -- Backbone --
...
def apply_mixup_cutmix(images, labels, num_classes, cfg):
    """Apply MixUp or CutMix to a batch. Returns mixed images and soft label pairs."""
    ...
    if np.random.rand() < cfg.mix_switch_prob:
        # CutMix
    ...
    for epoch in range(cfg.epochs):
        ...
        for step, (images, labels) in enumerate(pbar):
            ...
            images, labels_a, labels_b, lam = apply_mixup_cutmix(
                images, labels, cfg.num_pens, cfg)
            ...
            with torch.amp.autocast("cuda"):
                ...
                if labels_b is not None:
                    ...
                    loss = criterion(logits, labels_a) / cfg.grad_accum_steps
...
def run_inference(cfg, pen2idx):
    ...
    sub_path = cfg.output_dir / "submission_v12.csv"
    ...
    np.save(cfg.output_dir / "test_logits_v12.npy", all_logits)
    print(f"  Logits saved: {cfg.output_dir / 'test_logits_v12.npy'}")
