"""
CircleID — Pen Classification v15a
v9 + GeM Pooling — learnable generalized mean pooling

Key change from v9:
1. GeM pooling replaces default average pooling
   - Learnable parameter p (init=3.0) controls pooling sharpness
   - Better for fine-grained tasks like pen/ink texture discrimination
...
class CFG:
    ...
    output_dir: Path = BASE_DIR / "outputs_v15a"
    checkpoint_dir: Path = BASE_DIR / "checkpoints_v15a"
    num_pens: int = 8
    # -- Backbone --
...
class GeM(nn.Module):
    """Generalized Mean Pooling. p=1 is average pooling, p=inf is max pooling.
    Learns optimal p during training."""
...
class PenModel(nn.Module):
    def __init__(self, backbone_name, num_classes=8, drop_path_rate=0.2,
        ...
        print(f"  Pooling: GeM (learnable p, init=3.0)")
...
def run_inference(cfg, pen2idx):
    ...
    sub_path = cfg.output_dir / "submission_v15a.csv"
    ...
    np.save(cfg.output_dir / "test_logits_v15a.npy", all_logits)
    print(f"  Logits saved: {cfg.output_dir / 'test_logits_v15a.npy'}")
...
def main():
    ...
    print(f"CircleID Pen Classification v15a — ConvNeXt-V2 Tiny + GeM Pooling")
