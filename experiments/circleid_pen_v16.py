"""
CircleID — Pen Classification v16
EfficientNetV2-M + Focal Loss — different architecture for ensemble diversity

Pure CNN (inverted residuals + squeeze-excitation), completely different
from ConvNeXt/Swin/CAFormer. Pretrained on ImageNet-21k. 54M params.
...
class CFG:
    ...
    output_dir: Path = BASE_DIR / "outputs_v16"
    checkpoint_dir: Path = BASE_DIR / "checkpoints_v16"
    num_pens: int = 8
    # -- Backbone --
...
def run_inference(cfg, pen2idx):
    ...
    sub_path = cfg.output_dir / "submission_v16.csv"
    ...
    np.save(cfg.output_dir / "test_logits_v16.npy", all_logits)
    print(f"  Logits saved: {cfg.output_dir / 'test_logits_v16.npy'}")
...
def main():
    ...
    print(f"CircleID Pen Classification v16 — EfficientNetV2-M + Focal Loss")
