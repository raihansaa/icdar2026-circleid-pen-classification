"""
CircleID — Pen Classification v10
ConvNeXt-V2 Tiny + Focal Loss + Pseudo-labeling

Same as v9 but with pseudo-labeled test data added to training.
Pseudo-labels generated by best ensemble (0.9314 LB) at high confidence.
...
class CFG:
    ...
    output_dir: Path = BASE_DIR / "outputs_v10"
    checkpoint_dir: Path = BASE_DIR / "checkpoints_v10"
    pseudo_csv: str = "pseudo_labels.csv"
    num_pens: int = 8
    # -- Backbone --
...
def run_inference(cfg, pen2idx):
    ...
    sub_path = cfg.output_dir / "submission_v10.csv"
    ...
    np.save(cfg.output_dir / "test_logits_v10.npy", all_logits)
    print(f"  Logits saved: {cfg.output_dir / 'test_logits_v10.npy'}")
...
def main():
    ...
    print(f"CircleID Pen Classification v10 — ConvNeXt-V2 Tiny + Focal Loss + Pseudo-labels")
    ...
    # Load pseudo-labeled test data
    pseudo_df = None
    pseudo_path = BASE_DIR / cfg.pseudo_csv
    if pseudo_path.exists():
        pseudo_df = pd.read_csv(pseudo_path)
        pseudo_df = pseudo_df[pseudo_df["pen_id"].isin(pen_ids)].copy()
        # Ensure image_path column exists
        if "image_path" not in pseudo_df.columns and "image_id" in pseudo_df.columns:
            pseudo_df["image_path"] = "images/" + pseudo_df["image_id"] + ".png"
        # Add writer_id if missing
        if "writer_id" not in pseudo_df.columns:
            pseudo_df["writer_id"] = "PSEUDO"
        print(f"  Pseudo-labels: {len(pseudo_df)} images (threshold filtered)")
        print(f"    Per-pen: {pseudo_df['pen_id'].value_counts().sort_index().to_dict()}")
    else:
        print(f"  Pseudo-labels: not found at {pseudo_path}")
        print(f"  Run: python pseudo_label.py --generate")
    ...
    for fold_idx in folds_to_run:
        ...
        if pseudo_df is not None:
            fold_train_df = pd.concat([fold_train_df, pseudo_df], ignore_index=True)
        extra = (len(additional_df) if additional_df is not None else 0) + \
                (len(pseudo_df) if pseudo_df is not None else 0)
        if extra > 0:
            print(f"  Fold {fold_idx}: added {extra} extra samples "
