"""
CircleID — Pen Classification v5
DINOv2 + Expanded LoRA + Rich Ink Features + ArcFace + Same-Pen Mixup + K-Fold

Key changes from v4:
1. Resolution: 224 → 336 (576 patches vs 256 — finer stroke texture detail)
2. LoRA expanded: rank 32, 8 blocks, unfreeze last 4 blocks
3. Removed random erasing — it destroys ink pixels which ARE the signal
4. Same-pen mixup — interpolate same-pen-different-writer images during training
5. 60 epochs (was 40) — model was still improving
6. Richer ink features: 51D (was 28) — LBP texture, Gabor, connected components,
   intensity kurtosis/skewness, FFT, border sharpness
7. batch_size=16 (was 32) to fit 336px in GPU memory, grad_accum=8
...
class PenConfig:
    ...
    output_dir: Path = BASE_DIR / "outputs_v5b"
    checkpoint_dir: Path = BASE_DIR / "checkpoints_v5b"
    ...
    # ── Augmentation ─────────────────────────────────────────────────
    jitter_brightness: float = 0.08
    jitter_contrast: float = 0.08
...
def extract_ink_features(image, mode="cv2"):
    ...
    # 40-71: LBP histogram (32 bins — finer texture resolution)
    lbp = _compute_lbp(gray)
    lbp_ink = lbp[mask > 0]
    hist, _ = np.histogram(lbp_ink, bins=32, range=(0, 256))
    ...
    # 75: FFT low-frequency concentration
...
def apply_intensity_jitter(image, brightness_range=0.08, contrast_range=0.08,
                           saturation_range=0.05, hue_range=0.02, rng=None):
    if rng is None:
    ...
    # Saturation and hue jitter in HSV space
    if saturation_range > 0 or hue_range > 0:
        hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV).astype(np.float32)
        if saturation_range > 0:
            sf = 1.0 + rng.uniform(-saturation_range, saturation_range)
            hsv[:, :, 1] = np.clip(hsv[:, :, 1] * sf, 0, 255)
        if hue_range > 0:
            hf = rng.uniform(-hue_range, hue_range) * 180  # OpenCV hue range is 0-180
            hsv[:, :, 0] = (hsv[:, :, 0] + hf) % 180
        img = cv2.cvtColor(hsv.astype(np.uint8), cv2.COLOR_HSV2BGR)
    return img


def apply_gaussian_blur(image, sigma_range=(0.3, 1.0), rng=None):
    if rng is None:
        rng = np.random.default_rng()
...
def preprocess_image(image, mode="cv2", image_size=336,
                     apply_jitter=False, jitter_brightness=0.08, jitter_contrast=0.08,
                     jitter_saturation=0.05, jitter_hue=0.02,
                     rotation_deg=0.0, random_scale_crop=False,
                     crop_scale_range=(0.5, 1.0), hflip=False,
                     gaussian_blur_prob=0.0, gaussian_blur_sigma=(0.3, 1.0),
                     rng=None):
    if hflip:
    ...
    if apply_jitter:
        resized = apply_intensity_jitter(resized, jitter_brightness, jitter_contrast,
                                         jitter_saturation, jitter_hue, rng)
    if apply_jitter and gaussian_blur_prob > 0 and rng.random() < gaussian_blur_prob:
        resized = apply_gaussian_blur(resized, sigma_range=gaussian_blur_sigma, rng=rng)
...
class PenDataset(Dataset):
    def __init__(self, df, indices, data_dir, pen2idx, writer2idx,
                 ...
                 jitter_brightness=0.08, jitter_contrast=0.08,
                 jitter_saturation=0.05, jitter_hue=0.02,
                 rotation_deg=0.0, hflip_prob=0.5, crop_scale_range=(0.5, 1.0),
                 gaussian_blur_prob=0.0, gaussian_blur_sigma=(0.3, 1.0)):
        self.df = df
    ...
    def __getitem__(self, idx):
        ...
        final_image, mask_float = preprocess_image(
            ...
            hflip=do_hflip,
            gaussian_blur_prob=self.gaussian_blur_prob if self.is_training else 0.0,
...
def train_fold(fold, train_idx, val_idx, df, data_dir, pen2idx, writer2idx,
    ...
    train_ds = PenDataset(
        ...
        jitter_saturation=cfg.jitter_saturation, jitter_hue=cfg.jitter_hue,
...
if __name__ == "__main__":
    ...
    if cfg.seed != 42:
        cfg.checkpoint_dir = BASE_DIR / f"checkpoints_v5b_seed{cfg.seed}"
        cfg.output_dir = BASE_DIR / f"outputs_v5b_seed{cfg.seed}"
    set_seed(cfg.seed)

    print(f"\n{'=' * 70}")
    print(f"CircleID Pen v5 — 336px+LoRA32+RichFeats+SamePenMixup+{cfg.n_folds}-Fold")
    print(f"{'=' * 70}")
    ...
    print(f"  Jitter: b={cfg.jitter_brightness}, c={cfg.jitter_contrast}, "
          f"s={cfg.jitter_saturation}, h={cfg.jitter_hue}")
    print(f"  Gaussian blur: prob={cfg.gaussian_blur_prob}, sigma={cfg.gaussian_blur_sigma}")
    ...
    n_before = len(df)
    df = df[~df["writer_id"].isin(EXCLUDE_WRITERS)].reset_index(drop=True)
    print(f"\nExcluded writers {EXCLUDE_WRITERS}: {n_before} -> {len(df)} samples "
          f"(-{n_before - len(df)})")

    # Load additional training data (old test set)
    n_main = len(df)
    add_csv = data_dir / "additional_train.csv"
    if add_csv.exists():
        df_add = pd.read_csv(add_csv)
        df_add = df_add[~df_add["writer_id"].isin(EXCLUDE_WRITERS)].reset_index(drop=True)
        df_add = df_add[df_add["pen_id"].isin(df["pen_id"].unique())].reset_index(drop=True)
        df = pd.concat([df, df_add], ignore_index=True)
        print(f"Additional data: {len(df_add)} images (W41/W50 excluded)")
    additional_indices = list(range(n_main, len(df)))

    print(f"Train: {n_main} + {len(additional_indices)} additional = {len(df)}, "
          f"Test: {len(df_test)}, "
    ...
    # GroupKFold on main train data only (additional always goes to train)
    gkf = GroupKFold(n_splits=cfg.n_folds)
    main_df = df.iloc[:n_main]
    folds = list(gkf.split(range(n_main), main_df["pen_id"].values,
                            groups=main_df["writer_id"].values))
    ...
    if not args.infer_only:
        ...
        for fold_idx in folds_to_train:
            ...
            # Add additional data indices to training (never to validation)
            train_idx_with_add = np.concatenate([train_idx, additional_indices])
            acc, f1 = train_fold(fold_idx, train_idx_with_add, val_idx,
    ...
    np.save(Path(cfg.output_dir) / "test_logits_v5b.npy", final_logits)
    print(f"Logits saved: {Path(cfg.output_dir) / 'test_logits_v5b.npy'}")
    ...
    out_path = Path(cfg.output_dir) / "submission_pen_v5b.csv"
