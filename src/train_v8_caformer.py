"""
CircleID — Pen Classification v8
CAFormer-S36 + Focal Loss targeting pen 3/7 confusion

Key changes from v7:
1. CAFormer-S36 backbone (conv+attention hybrid, IN22K, native 384px)
2. Focal Loss (gamma=2.0) — downweights easy pens (1,2,4,5), focuses on hard pens (3,7)
3. Backbone LR 3e-5 (higher than v7's 1e-5 for proper full finetune)
4. All v7 fixes: W41/W50 excluded, no vertical flip, no RandomErasing, gaussian blur
...
class CFG:
    ...
    output_dir: Path = BASE_DIR / "outputs_v8"
    checkpoint_dir: Path = BASE_DIR / "checkpoints_v8"
    num_pens: int = 8
    # -- Backbone --
    backbone: str = "caformer_s36.sail_in22k_ft_in1k_384"
    image_size: int = 336
...
def run_inference(cfg, pen2idx):
    ...
    sub_path = cfg.output_dir / "submission_v8.csv"
    ...
    np.save(cfg.output_dir / "test_logits_v8.npy", all_logits)
    print(f"  Logits saved: {cfg.output_dir / 'test_logits_v8.npy'}")
...
def main():
    ...
    print(f"CircleID Pen Classification v8 — CAFormer + Focal Loss")
