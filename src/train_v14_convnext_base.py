"""
CircleID — Pen Classification v14
ConvNeXt-V2 Base (89M) + Focal Loss + Gradient Checkpointing

Key changes from v9:
1. ConvNeXt-V2 Base backbone (89M params, 3x larger than Tiny)
2. Gradient checkpointing to fit on 8GB VRAM
3. Batch size 8 + grad_accum 8 = effective batch 64
4. Saves OOF predictions for CV-optimized ensemble weights
...
class CFG:
    ...
    output_dir: Path = BASE_DIR / "outputs_v14"
    checkpoint_dir: Path = BASE_DIR / "checkpoints_v14"
    num_pens: int = 8
    # -- Backbone --
...
class PenModel(nn.Module):
    def __init__(self, backbone_name, num_classes=8, drop_path_rate=0.2,
                 head_dropout=0.3, use_grad_checkpoint=True):
        super().__init__()
    ...
    for epoch in range(cfg.epochs):
        ...
        with torch.no_grad():
            for images, labels in val_loader:
                ...
                with torch.amp.autocast("cuda"):
                    logits = model(images)
                val_logits_list.append(logits.cpu().float().numpy())
        ...
        if val_f1 > best_f1:
            ...
            # Save OOF logits for this fold's best
            val_logits_np = np.concatenate(val_logits_list, axis=0)
            np.save(ckpt_path / "oof_logits.npy", val_logits_np)
            np.save(ckpt_path / "oof_labels.npy", np.array(val_labels))
            np.save(ckpt_path / "oof_indices.npy", np.array(val_df.index.tolist()))
        else:
            patience_counter += 1
...
def run_inference(cfg, pen2idx):
    ...
    for fold in range(cfg.n_folds):
        ...
        model = PenModel(cfg.backbone, cfg.num_pens, cfg.drop_path_rate, cfg.head_dropout,
                         use_grad_checkpoint=False)  # no checkpointing at inference
    ...
    sub_path = cfg.output_dir / "submission_v14.csv"
    ...
    np.save(cfg.output_dir / "test_logits_v14.npy", all_logits)
    print(f"  Logits saved: {cfg.output_dir / 'test_logits_v14.npy'}")


# ══════════════════════════════════════════════════════════════════════════
# Multi-Seed Inference — averages predictions across seeds
# ══════════════════════════════════════════════════════════════════════════

def run_multiseed_inference(cfg, seed_list):
    print(f"\n{'='*60}")
    print(f"  MULTI-SEED INFERENCE — seeds: {seed_list}")
    print(f"{'='*60}")

    # Load data for pen mapping
    ...
    for seed in seed_list:
        seed_ckpt_dir = BASE_DIR / f"checkpoints_v14_seed{seed}"
        print(f"\n  --- Seed {seed} ---")

        for fold in range(cfg.n_folds):
            ckpt_path = seed_ckpt_dir / f"fold{fold}" / "best_model.pt"
            if not ckpt_path.exists():
            ...
            model = PenModel(cfg.backbone, cfg.num_pens, cfg.drop_path_rate,
                             cfg.head_dropout, use_grad_checkpoint=False)
            state = ckpt["model"]
            if "ema_shadow" in ckpt:
                for n in ckpt["ema_shadow"]:
                    if n in state:
            ...
            print(f"    Fold {fold}: val_f1={ckpt.get('val_f1', 0):.4f}, epoch={ckpt.get('epoch', '?')}")
    ...
    all_logits /= total_models
    predictions = np.argmax(all_logits, axis=1)
    pen_preds = [idx2pen[p] for p in predictions]

    out_dir = BASE_DIR / "outputs_v14_multiseed"
    out_dir.mkdir(parents=True, exist_ok=True)

    seeds_str = "_".join(str(s) for s in seed_list)
    submission = pd.DataFrame({
        "image_id": test_df["image_id"].values,
        "pen_id": pen_preds,
    })
    sub_path = out_dir / f"submission_v14_seeds{seeds_str}.csv"
    submission.to_csv(sub_path, index=False)
    print(f"\n  Submission saved: {sub_path}")
    print(f"  Total models averaged: {total_models} ({len(seed_list)} seeds x {cfg.n_folds} folds)")
    print(f"  TTA: H-flip={'yes' if cfg.tta_hflip else 'no'}")
    print(f"\n  Pen distribution:")
    print(submission["pen_id"].value_counts().sort_index().to_string())

    np.save(out_dir / f"test_logits_v14_seeds{seeds_str}.npy", all_logits)
    print(f"  Logits saved for ensemble blending")
...
def main():
    ...
    parser.add_argument("--seed", type=int, default=None, help="Random seed (default: 42). Use different seeds for multi-seed ensemble")
    parser.add_argument("--seeds", type=str, default=None, help="Comma-separated seeds for multi-seed inference, e.g. '42,43,44'")
    ...
    # Per-seed checkpoint and output dirs
    cfg.checkpoint_dir = BASE_DIR / f"checkpoints_v14_seed{cfg.seed}"
    cfg.output_dir = BASE_DIR / f"outputs_v14_seed{cfg.seed}"

    # Multi-seed inference mode
    if args.seeds and args.infer_only:
        seed_list = [int(s.strip()) for s in args.seeds.split(",")]
        run_multiseed_inference(cfg, seed_list)
        return
    ...
    print(f"CircleID Pen Classification v14 — ConvNeXt-V2 Base + Focal Loss")
