"""
CircleID — Pen Classification v11
Swin-Base (IN22K ft IN1K, hierarchical shifted-window attention, 87M params)

Key: Maximally different architecture from existing models:
  - v9: ConvNeXt-V2 Tiny (pure CNN, 28M)
  - v8: CAFormer-S36 (conv+attention hybrid, 36M)
  - v5b: DINOv2 ViT (self-supervised ViT + ink features)
  - v11: Swin-Base (hierarchical ViT with shifted windows, 87M, IN22K→IN1K)
...
class CFG:
    ...
    output_dir: Path = BASE_DIR / "outputs_v11"
    checkpoint_dir: Path = BASE_DIR / "checkpoints_v11"
    num_pens: int = 8
    # -- Backbone --
...
class PenModel(nn.Module):
    def __init__(self, backbone_name, num_classes=8, drop_path_rate=0.15,
                 head_dropout=0.3, image_size=None):
        super().__init__()
        kwargs = dict(
            pretrained=True,
            num_classes=0,
...
def train_one_fold(fold, train_df, val_df, cfg, pen2idx):
    ...
    model = PenModel(cfg.backbone, cfg.num_pens, cfg.drop_path_rate, cfg.head_dropout, cfg.image_size)
    model.to(cfg.device)
    ...
    # Reload best and print per-pen accuracy
...
def run_inference(cfg, pen2idx):
    ...
    sub_path = cfg.output_dir / "submission_v11.csv"
    ...
    np.save(cfg.output_dir / "test_logits_v11.npy", all_logits)
    print(f"  Logits saved: {cfg.output_dir / 'test_logits_v11.npy'}")
...
def run_multiseed_inference(cfg, seed_list):
    ...
    idx2pen = {i: p for i, p in enumerate(pen_ids)}
    ...
    for seed in seed_list:
        if seed == 42:
            seed_ckpt_dir = BASE_DIR / "checkpoints_v11"
        else:
            seed_ckpt_dir = BASE_DIR / f"checkpoints_v11_seed{seed}"
        print(f"\n  --- Seed {seed} ({seed_ckpt_dir}) ---")
        ...
        for fold in range(cfg.n_folds):
            ...
            model = PenModel(cfg.backbone, cfg.num_pens, cfg.drop_path_rate,
                             cfg.head_dropout, cfg.image_size)
            state = ckpt["model"]
            if "ema_shadow" in ckpt:
                for n in ckpt["ema_shadow"]:
                    if n in state:
    ...
    out_dir = BASE_DIR / "outputs_v11_multiseed"
    ...
    sub_path = out_dir / f"submission_v11_seeds{seeds_str}.csv"
    submission.to_csv(sub_path, index=False)
    print(f"\n  Submission saved: {sub_path}")
    print(f"  Total models averaged: {total_models}")

    np.save(out_dir / f"test_logits_v11_seeds{seeds_str}.npy", all_logits)
...
def main():
    ...
    parser.add_argument("--seeds", type=str, default=None, help="Comma-separated seeds for multi-seed inference")
    ...
    # Per-seed dirs (seed 42 uses original dirs for backward compat)
    if cfg.seed != 42:
        cfg.checkpoint_dir = BASE_DIR / f"checkpoints_v11_seed{cfg.seed}"
        cfg.output_dir = BASE_DIR / f"outputs_v11_seed{cfg.seed}"
    ...
    print(f"CircleID Pen Classification v11 — Swin-Base")
