"""
CircleID — Pen Classification v9
ConvNeXt-V2 Tiny + Focal Loss — best backbone with targeted loss

Key changes from v8:
1. ConvNeXt-V2 Tiny backbone (proven best at LB 0.9214 with standard CE)
2. Focal Loss (gamma=2.0) — same as v8, targets pen 3/7
3. Backbone LR 1.5e-5 (validated as best in v8 fold 2)
4. All fixes: W41/W50 excluded, no vertical flip, no RandomErasing, gaussian blur
...
class CFG:
    ...
    output_dir: Path = BASE_DIR / "outputs_v9"
    checkpoint_dir: Path = BASE_DIR / "checkpoints_v9"
    num_pens: int = 8
    # -- Backbone --
...
class FocalLoss(nn.Module):
    ...
    With gamma=2.0, an example classified with 95% confidence gets 400x less
    weight than one at 50% confidence. This forces the model to spend gradient
    budget on pen 3/7 instead of already-perfect pens 1,2,4,5."""
...
def run_inference(cfg, pen2idx):
    ...
    sub_path = cfg.output_dir / "submission_v9.csv"
    ...
    np.save(cfg.output_dir / "test_logits_v9.npy", all_logits)
    print(f"  Logits saved: {cfg.output_dir / 'test_logits_v9.npy'}")
...
def run_multiseed_inference(cfg, seed_list):
    ...
    for seed in seed_list:
        if seed == 42:
            seed_ckpt_dir = BASE_DIR / "checkpoints_v9"
        else:
            seed_ckpt_dir = BASE_DIR / f"checkpoints_v9_seed{seed}"
    ...
    out_dir = BASE_DIR / "outputs_v9_multiseed"
    ...
    sub_path = out_dir / f"submission_v9_seeds{seeds_str}.csv"
    submission.to_csv(sub_path, index=False)
    print(f"\n  Submission saved: {sub_path}")
    print(f"  Total models averaged: {total_models}")

    np.save(out_dir / f"test_logits_v9_seeds{seeds_str}.npy", all_logits)
...
def main():
    ...
    if cfg.seed != 42:
        cfg.checkpoint_dir = BASE_DIR / f"checkpoints_v9_seed{cfg.seed}"
        cfg.output_dir = BASE_DIR / f"outputs_v9_seed{cfg.seed}"
    ...
    print(f"CircleID Pen Classification v9 — ConvNeXt-V2 Tiny + Focal Loss")
